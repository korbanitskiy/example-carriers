...
class NaqelInvoicePDF(BaseInvoicePDF):
    ...
    def _invoice_items_table(self):
        ...
        if discount_applied:
            ...
            totals_rows.append([self._cost_format("Goods value after discount", order_vat_custom, total_style, self.order.currency), '', '', '', '', ''])
        else:
            totals_rows.append([self._cost_format("Goods value", order_vat_custom, total_style, self.order.currency), '', '', '', '', ''])
        totals_rows.append([self._cost_format("Total shipping cost", total_shipping_cost, total_style, self.order.currency), '', '', '', '', ''])
        totals_rows.append([self._cost_format("Customs duty", customs_duty, total_style, self.order.currency), '', '', '', '', ''])
        totals_rows.append([self._cost_format("COD Charges", self.shipment.totals.cash_on_delivery, total_style, self.order.currency), '', '', '', '', ''])
        totals_rows.append([self._cost_format("Domestic delivery charge", domestic_delivery_charge, total_style, self.order.currency), '', '', '', '', ''])
        totals_rows.append([self._cost_format("Shipment Declared Value", order_vat_custom, total_style, self.order.currency), '', '', '', '', ''])
