...
from .api import AramexOrderAPI
from .pdf import click_and_collect, home_collection
from .pdf.invoice import EgyptInvoicePDF, SaudiArabiaInvoicePDF, ArabEmiratesInvoicePDF
from ..abc.pdf import EnigmoInvoicePDF, BaseInvoicePDF
from ..abc.shipping import AbstractShippingService, TrackingNumberMixin


class AramexShippingService(TrackingNumberMixin, AbstractShippingService):
    carrier_name = enum.CarrierName.aramex
    invoice_pdf_creators = {
        'EG': EgyptInvoicePDF,
        'SA': SaudiArabiaInvoicePDF,
        'AE': ArabEmiratesInvoicePDF,
    }
    ...
    def can_send_shipment(self, shipment):
        return shipment.delivery_type == enum.DeliveryType.international and self.tracking_number_qs.count() > 0

    def create_shipping_document(self, shipment):
        creators = {
            enum.ShippingMethod.home_collection: home_collection.ShipmentPDF,
            enum.ShippingMethod.click_and_collect: click_and_collect.ShipmentPDF,
        }
        pdf_creator = creators[shipment.order.shipping_method_type](self.settings, shipment)
        return pdf_creator.create_document()

    def create_invoice_document(self, shipment):
        if shipment.order.channel.code == 'enigmo':
            pdf_creator = EnigmoInvoicePDF(self.settings, shipment)
        else:
            pdf_creator = self.invoice_pdf_creators.get(shipment.order.shipping_address.country, BaseInvoicePDF)(self.settings, shipment)
        return pdf_creator.create_document()
    ...
    def _send_shipping_information(self, shipment):
        ...
        api = AramexOrderAPI(self.settings)
    ...
    def _create_shipping_xml(self, shipment):
        ...
        if shipment.order.shipping_method_type == enum.ShippingMethod.click_and_collect:
            shipping_xml = ClickAndCollectShippingXML(shipment, waybill_document, self.carrier_settings)
        else:
...
class ClickAndCollectShippingXML(HomeCollectionShippingXML):

    def __init__(self, shipment, waybill_document, carrier_settings):
        super().__init__(shipment, waybill_document, carrier_settings)
        self.entity_id = self.carrier_settings.cc_entity_id or self.carrier_settings.entity_id
        self.entity_pin = self.carrier_settings.cc_entity_pin or self.carrier_settings.entity_pin
        self.account_number = self.carrier_settings.cc_account_number or self.carrier_settings.account_number
        self.account_post_code = self.carrier_settings.cc_account_post_code or self.carrier_settings.account_post_code

    def _consignee_info(self):
        address = [
            self.order.shipping_address.address,
            self.order.shipping_info.get('description')
        ]
        return [
            E.ConsigneeName(self.order.shipping_address.full_name),
            E.ConsigneeAddress(', '.join(filter(None, address))),
            E.ConsigneeCity(self.order.shipping_address.city),
            E.ConsigneeZipCode(self.order.shipping_address.postcode or ''),
            E.ConsigneeCountryCode(self.order.shipping_address.country),
            E.ConsigneeTelephone(self.order.shipping_address.phone),
            E.ConsTelephone2(self.order.shipping_address.fax or ''),
            ...
            E.ConsLongitude(self.order.shipping_address.longitude or ''),
            E.ConsigneeEmail(self.order.email or ''),
            E.ConsigneeReference(self.order.shipping_info['code']),
            E.ConsigneeReference2(self.order.email or ''),
            E.AttentionOf(self.order.shipping_address.full_name),
        ]
